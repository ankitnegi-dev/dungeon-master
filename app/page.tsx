...
export default function DungeonMaster() {
  ...
  const speakNarration = (text: string) => {
    ...
    const deepVoice = voices.find(
      (v) => v.name.includes("Male") || v.name.includes("David") || v.name.includes("Daniel")
    );
  ...
  const captureObject = async () => {
    ...
    closeCamera();
    const storyContext = messages.slice(-3).map((m) => m.text).join(" ");
    try {
  ...
  // Corner decorations
  const corners = [[14, 14], [196, 14], [14, 283], [196, 283]];
  corners.forEach(([x, y]) => {
    doc.setFontSize(10);
  ...
  doc.text(worldNames[world] || "An Adventure Chronicle", pageW / 2, 135, { align: "center" });
  ...
  doc.text(`Final Health: ${stats.health}/100  ·  Gold: ${stats.gold}  ·  Location: ${stats.location}`, pageW / 2, 248, { align: "center" });
  ...
  doc.text("Powered by Gemini AI · AI Dungeon Master", pageW / 2, 278, { align: "center" });

  // ── Story pages ──
  const dmMessages = messages.filter((m: any) => m.role === "dm" && m.text && m.text.length > 10);
...
const clearSession = () => {
      ...
      for (const line of lines) {
        try {
          ...
          if (data.stats) setStats({
            ...
            inventory: Array.isArray(data.stats.inventory) ? data.stats.inventory : [],
          });
  ...
  const startAdventure = () => { setStarted(true); sendMessage("Begin the adventure."); };
  const healthColor = stats.health > 60 ? "#c84b2a" : stats.health > 30 ? "#d4521a" : "#8b1a1a";

  return (
    <div className="grimoire-bg" style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center" }}>
      <div style={{ width: "100%", maxWidth: "1100px", display: "flex", flexDirection: "column", height: "100vh" }}>
        ...
        <header style={{
          ...
          alignItems: "center",
          gap: "12px",
          background: "linear-gradient(180deg, rgba(201,148,58,0.03) 0%, transparent 100%)",
        }}>
          <div className="orb-pulse" style={{ width: "8px", height: "8px", borderRadius: "50%", background: "radial-gradient(circle, #e8b96a, #c9943a)" }} />
          <h1 className="font-display title-shimmer" style={{ fontSize: "18px", letterSpacing: "0.05em" }}>
            AI Dungeon Master
          </h1>
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: "8px" }}>
            <span style={{ fontSize: "11px", color: "var(--text-dim)", fontFamily: "Cinzel, serif", letterSpacing: "0.1em" }}>
              POWERED BY GEMINI
            </span>
            <div style={{ width: "1px", height: "12px", background: "var(--border)" }} />
            <span style={{ fontSize: "11px", color: "var(--gold-dim)", fontFamily: "Cinzel, serif", letterSpacing: "0.08em" }}>
              {stats.location}
            </span>
          </div>
          {started && (
    ...
    📖 EXPORT
        ...
        <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
            <div className="scroll-area" style={{ flex: 1, overflowY: "auto", padding: "24px 20px", display: "flex", flexDirection: "column", gap: "20px" }}>

              {!started && (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: "100%", gap: "28px", padding: "20px" }}>
    {!world ? (
      <>
        <div className="landing-rune">⚔</div>
        <div style={{ textAlign: "center" }}>
          <h2 className="font-display" style={{ fontSize: "24px", marginBottom: "8px" }}>
            <span className="title-shimmer">Choose Your Realm</span>
          </h2>
          <p className="font-body" style={{ color: "var(--text-secondary)", fontSize: "16px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", width: "100%", maxWidth: "600px" }}>
          {[
            { id: "fantasy", icon: "⚔️", name: "Dark Fantasy", sub: "Swords, sorcery & ancient evil", color: "#c9943a", border: "rgba(201,148,58,0.4)", bg: "rgba(201,148,58,0.05)" },
            { id: "scifi", icon: "🚀", name: "Space Opera", sub: "Neon cities & alien worlds", color: "#5a9aec", border: "rgba(90,154,236,0.4)", bg: "rgba(90,154,236,0.05)" },
            { id: "horror", icon: "🕯️", name: "Cosmic Horror", sub: "Sanity frays, ancient things stir", color: "#9a5aec", border: "rgba(154,90,236,0.4)", bg: "rgba(154,90,236,0.05)" },
            { id: "samurai", icon: "🗡️", name: "Feudal Japan", sub: "Honor, betrayal & bushido", color: "#ec5a5a", border: "rgba(236,90,90,0.4)", bg: "rgba(236,90,90,0.05)" },
          ].map((w) => (
            <button
              key={w.id}
              ...
              onMouseEnter={e => {
                (e.currentTarget as HTMLElement).style.background = w.bg.replace('0.05', '0.12');
                (e.currentTarget as HTMLElement).style.borderColor = w.color;
                (e.currentTarget as HTMLElement).style.boxShadow = `0 0 24px ${w.border}`;
              }}
              onMouseLeave={e => {
            ...
            >
              ...
              <p style={{ fontFamily: "Cinzel, serif", fontSize: "13px", color: w.color, marginBottom: "4px", letterSpacing: "0.05em" }}>{w.name}</p>
              <p style={{ fontFamily: "EB Garamond, serif", fontSize: "13px", color: "var(--text-dim)", fontStyle: "italic" }}>{w.sub}</p>
            </button>
          ))}
        </div>
      </>
      ...
      <>
        <div style={{ fontSize: "60px" }}>
          {world === 'fantasy' ? '⚔️' : world === 'scifi' ? '🚀' : world === 'horror' ? '🕯️' : '🗡️'}
        </div>
        <div style={{ textAlign: "center" }}>
          <h2 className="font-display" style={{ fontSize: "22px", marginBottom: "8px" }}>
            <span className="title-shimmer">
              {world === 'fantasy' ? 'Dark Fantasy' : world === 'scifi' ? 'Space Opera' : world === 'horror' ? 'Cosmic Horror' : 'Feudal Japan'}
            </span>
          </h2>
          <p className="font-body" style={{ color: "var(--text-secondary)", fontSize: "16px", maxWidth: "340px", lineHeight: "1.7" }}>
            {world === 'fantasy' && 'A world of shadows and sorcery awaits. Ancient evil stirs in the darkness.'}
            {world === 'scifi' && 'The galaxy holds no mercy. Your story begins at the edge of known space.'}
            {world === 'horror' && 'Something watches from beyond the veil. Your sanity is your only shield.'}
            {world === 'samurai' && 'The way of the sword demands everything. Honor is your only currency.'}
          </p>
        </div>
        <div style={{ display: "flex", gap: "12px" }}>
          <button
            ...
            className="act-btn"
            style={{ padding: "14px 40px", borderRadius: "4px", fontSize: "14px", letterSpacing: "0.15em" }}
          >
            ENTER THE REALM
          </button>
          <button
            onClick={() => setWorld('')}
            className="mic-btn"
            style={{ padding: "14px 20px", borderRadius: "4px", fontSize: "13px", border: "1px solid", fontFamily: "Cinzel, serif", letterSpacing: "0.1em" }}
          >
            BACK
          </button>
        </div>
                ...
                <div key={i} className="msg-appear" style={{
                  ...
                  {msg.role === "dm" && (
                    <div style={{
                      ...
                      background: "radial-gradient(circle at 40% 35%, #3a2810, #1a0f05)",
                      border: "1px solid var(--gold-dim)",
                  ...
                  <div
                    className={msg.role === "dm" ? "dm-bubble" : "player-bubble"}
                    style={{
                      maxWidth: "88%",
                      borderRadius: msg.role === "dm" ? "2px 12px 12px 12px" : "12px 2px 12px 12px",
                      padding: "14px 18px",
                    }}
                  >
                    {msg.text || (loading && i === messages.length - 1 ? (
                      <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                        <span className="font-body" style={{ color: "var(--text-dim)", fontStyle: "italic", fontSize: "15px" }}>The spirits stir</span>
                  ...
                  {msg.role === "player" && (
                    <div style={{
                      width: "28px", height: "28px", borderRadius: "50%",
                      background: "radial-gradient(circle at 40% 35%, #2a1010, #150808)",
                      border: "1px solid rgba(139,26,26,0.5)",
                      display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: "12px", flexShrink: 0, marginTop: "2px",
                    }}>🧙</div>
            ...
            {started && (
              <div style={{
                ...
                padding: "14px 16px",
                background: "linear-gradient(0deg, rgba(201,148,58,0.02) 0%, transparent 100%)",
              }}>
                <div style={{ display: "flex", gap: "8px" }}>
                  <button
                    ...
                    style={{ padding: "10px 12px", borderRadius: "6px", fontSize: "16px", border: "1px solid" }}
                  ...
                  <button
                    ...
                    style={{ padding: "10px 12px", borderRadius: "6px", fontSize: "16px", border: "1px solid", opacity: cameraCapturing ? 0.5 : 1 }}
                  ...
                  <input
                    ...
                    placeholder={isListening ? "Listening to your command..." : "What do you do, traveller?"}
                    disabled={loading || isListening}
                ...
                <p style={{ fontSize: "11px", color: "var(--text-dim)", marginTop: "8px", textAlign: "center", fontFamily: "Cinzel, serif", letterSpacing: "0.08em" }}>
          ...
          {started && (
            <div style={{
              ...
              overflow: "hidden",
              background: "linear-gradient(180deg, rgba(10,8,5,0.6) 0%, transparent 100%)",
            }}>
              <div className="scene-frame" style={{ height: "180px", background: "var(--parchment)", flexShrink: 0 }}>
                {imageLoading && (
                  <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: "8px", zIndex: 1 }}>
                    <div className="spinner" style={{ width: "24px", height: "24px", border: "2px solid var(--gold-dim)", borderTopColor: "var(--gold)", borderRadius: "50%" }} />
                    <span className="font-heading" style={{ fontSize: "10px", color: "var(--gold-dim)", letterSpacing: "0.15em" }}>PAINTING SCENE</span>
                  </div>
                )}
                {sceneImage ? (
                  <img
                    src={sceneImage}
                    alt="Scene"
                    style={{ width: "100%", height: "100%", objectFit: "cover", display: imageLoading ? "none" : "block", filter: "sepia(20%) contrast(1.1)" }}
                    onLoad={() => setImageLoading(false)}
                    onError={() => setImageLoading(false)}
                  />
                ) : !imageLoading ? (
                  <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <span style={{ fontSize: "11px", color: "var(--text-dim)", fontFamily: "Cinzel, serif", letterSpacing: "0.1em" }}>AWAITING VISION</span>
                  </div>
                ) : null}
              </div>
              ...
              <div className="scroll-area" style={{ flex: 1, overflowY: "auto", padding: "16px" }}>
                <div style={{ marginBottom: "16px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                    ...
                    <span style={{ fontFamily: "Cinzel, serif", fontSize: "12px", color: healthColor }}>{stats.health} / 100</span>
                  </div>
                  <div style={{ height: "4px", background: "rgba(255,255,255,0.05)", borderRadius: "2px", overflow: "hidden" }}>
                    <div className="health-bar" style={{ height: "100%", width: `${stats.health}%`, borderRadius: "2px" }} />
                  </div>
                </div>
                ...
                <div style={{ marginBottom: "16px" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "baseline", gap: "6px", marginTop: "4px" }}>
                    <span style={{ fontFamily: "Cinzel, serif", fontSize: "20px", color: "var(--gold)" }}>{stats.gold}</span>
                    <span style={{ fontFamily: "Cinzel, serif", fontSize: "11px", color: "var(--gold-dim)" }}>PIECES</span>
                  </div>
                </div>
                <hr className="divider" style={{ marginBottom: "16px" }} />
                <div>
                  <span className="stat-label" style={{ display: "block", marginBottom: "8px" }}>Satchel</span>
                  {(stats.inventory ?? []).length === 0 ? (
                    <p style={{ fontFamily: "EB Garamond, serif", fontSize: "14px", color: "var(--text-dim)", fontStyle: "italic" }}>Nothing carried</p>
                  ) : (
                    <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                      {stats.inventory.map((item, i) => (
                        <div key={i} className="inv-item" style={{ padding: "7px 10px", borderRadius: "3px", fontFamily: "EB Garamond, serif", fontSize: "14px", color: "var(--text-secondary)", display: "flex", alignItems: "center", gap: "8px" }}>
      ...
      {cameraOpen && (
        <div style={{
          ...
          <p style={{ fontFamily: "Cinzel Decorative, serif", fontSize: "16px", color: "var(--gold)", letterSpacing: "0.15em" }}>
            REVEAL YOUR OBJECT
          </p>
          <p style={{ fontFamily: "EB Garamond, serif", fontSize: "15px", color: "var(--text-secondary)", fontStyle: "italic" }}>
            Hold it before the arcane lens...
          </p>
          <div style={{ position: "relative", borderRadius: "8px", overflow: "hidden", border: "1px solid var(--gold-dim)", boxShadow: "0 0 60px rgba(201,148,58,0.15)" }}>
            <video
              ref={videoRef}
          ...
          <div style={{ display: "flex", gap: "12px" }}>
            <button
              ...
              className="act-btn"
              style={{ padding: "12px 32px", borderRadius: "6px", fontSize: "13px", letterSpacing: "0.15em" }}
            >
              ✦ REVEAL TO THE DM
            </button>
            <button
              onClick={closeCamera}
              className="mic-btn"
              style={{ padding: "12px 20px", borderRadius: "6px", fontSize: "13px", border: "1px solid" }}
            >
              CANCEL
            </button>
          </div>
          <p style={{ fontSize: "11px", color: "var(--text-dim)", fontFamily: "Cinzel, serif", letterSpacing: "0.08em" }}>
